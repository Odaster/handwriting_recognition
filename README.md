...
- **Коррекция текста** (опционально):
  - **SAGE** (`ai-forever/sage-fredt5-distilled-95m`) — контекстный корректор орфографии/грамматики/пунктуации;
...
- **Контекстная коррекция (SAGE)** исправляет орфографию/грамматику/пунктуацию с учётом
  контекста (например «исли изти» → «Если идти») и не ломает верные слова. Не всесильна:
  если OCR-ошибка слишком далека от правильного слова, модель может её не восстановить.
